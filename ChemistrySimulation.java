...
public class ChemistrySimulation {
	...
	for (int step=1; step<=numSteps; step++) {
	    ...
	    move ();       // This will randomly move each molecule.
	    react ();      // This is where we'll apply the rules.
	    draw ();       // We'll draw circles here.
    ...
    static void react () 
    {
	// First we'll see if a typeC is to decompose into a typeA or typeC.

	// We will add those marked for decomposition by placing them in a list:
	LinkedList<Molecule> removeListC = new LinkedList<> ();
	...
	for (Molecule c: typeC) {
		...
		// Add this particular c to the "remove" list 
		// but DO NOT remove it here, as in typeC.remove(c).
		removeListC.add (c);
	    }
	}
	...
	// For each red, see if it's near a green to form a yellow.
	// Again, we'll place removals in a list.
	LinkedList<Molecule> removeListA = new LinkedList<> ();
	...
	for (Molecule a: typeA) {
	    ...
	    if (b != null) {	               // We found a red-green pair.
	...
	// Now remove those that need removing.
	for (Molecule c: removeListC) {
	    typeC.remove (c);
	}

	for (Molecule a: removeListA) {
	    typeA.remove (a);
	}

	// Why did we do removals later?
    }
	...
	for (Molecule c: typeC) {
	    DrawTool.setCircleColor ("orange");
